...
% IFDREAD read IFD entries from LSM file according to TIFF standard (since
% the LSM format is just an extension to the Tiff standard in the end)
...
if fseek(fid, 8, 'bof')
  error(['LSMReader.ifdread: Received error on file seek to offset 8: ' ferror(fid)]);
end
...
for i = 1:numEntries
  ...
  if fseek(fid, entriesStartPos + ((i - 1) * ENTRY_LENGTH), 'bof')
    error(['LSMReader.ifdread: Received error on file seek to position for entry ' num2str(i) ': ' ferror(fid)]);
  end
  ...
  switch ifd(i).typecode
    ...
    case {5, 10, 12}    % uint32 / uint32, int32 / int32, float64
      ifd(i).bytes = 8;
    otherwise
      error(['LSMReader.ifdread: Illegal typecode: ' num2str(ifd(i).typecode) '.  Misformed IFD']);
  end
  ...
  if ((ifd(i).bytes * ifd(i).count) > 4)
    ...
    if fseek(fid, ifd(i).offset, 'bof')
      error(['LSMReader.ifdread: Received error on file seek to data for entry ' num2str(i) '(' num2str(ifd(i).offset) '): ' ferror(fid)]);
    end
  end
